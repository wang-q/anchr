...
- 启动并行/长任务前先估算峰值内存（单进程实测 × 并发数），对照 `free`
  确认；不确定时先串行跑一个测峰值。
- 未经用户确认，并发内存型任务合计不超过机器内存的 1/2。

### 错装判定纪律

- 错装 ground truth **以 quast 等权威全比对为准**；探针命中、覆盖形态等
  间接证据只能用做假设来源，不能替代权威判定（探针取自重复区时会误报
  "嵌合"——本次 7 个 unitig 单比对参考、quast 0 mis，均为探针误报）。

### 基准数据集管理

- 数据集定义、运行方式、质量基线以 `results/model_org.md` 为唯一权威
  记录；引用基线必须来自该文件，不依赖记忆或 /tmp 残留。
- quast 对比参数保持与该文件既有表格一致，禁止随意变更导致纵向对比失效。
- 实验脚本与原始报告可放 /tmp，但结论性数字确认后应追加进该文件。
