//! Set of coverage tiers (`Coverage`), vendored from the external `intspan`
//! crate (kept API-identical so the two can be swapped without touching
//! call sites). Uses `pgr::libs::ds::IntSpan` for the underlying spans.
...
impl Coverage {
    ...
    /// ```
    /// # use anchr::libs::coverage::Coverage;
