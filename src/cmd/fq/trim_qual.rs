...
pub fn execute(args: &ArgMatches) -> anyhow::Result<()> {
    ...
    for out in [Some(outfile), outfile_2, outfile_single]
        ...
        .flatten()
    {
        crate::cmd::args::ensure_outfile_distinct(out, infiles.iter().map(String::as_str))?;
    }
    ...
    if infiles.len() == 1 {
        ...
        crate::libs::fq::trim::run_single(&infiles[0], &mut out, &opts)?;
        out.flush()?;
    } else {
