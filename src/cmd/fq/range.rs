...
"###,
        ...
        .arg(crate::cmd::args::ranges_arg())
        .arg(crate::cmd::args::rgfile_arg())
        .arg(
            Arg::new("cache")
                .long("cache")
                .short('c')
...
pub fn execute(args: &ArgMatches) -> anyhow::Result<()> {
    ...
    crate::cmd::args::ensure_outfile_distinct(outfile, protected.iter().map(|s| s.as_str()))?;
    if let Some(o2) = outfile_2 {
        crate::cmd::args::ensure_outfile_distinct(o2, protected.iter().map(|s| s.as_str()))?;
        if outfile != "stdout" && pgr::libs::io::same_path(outfile, o2) {
            bail!("output files must be distinct: {} and {}", outfile, o2);
        }
    }

    let ranges = crate::cmd::args::collect_ranges(args)?;
