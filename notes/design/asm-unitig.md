# `anchr asm unitig`：cdBG unitig 压实与计数基础设施（设计）

> 2026-08-11 起记录，2026-08-17 自 `asm-assemble.md` §8-13 拆出。
> 内容：`asm unitig` 命令语义（BCALM graph3 式最大 unitig 压实，§8）、
> bcalm 后处理语义分析与功能迁移（§10）、大规模性能基准与优化（§9）、
> DFA 状态分类 / supermer 两段计数 / 分桶调研（§11-§13）。
> tadpole contigMode 迁移见 [asm-contig.md](asm-contig.md)；
> 参考：[bcalm.md](../references/bcalm.md)、
> [cuttlefish.md](../references/cuttlefish.md)。

## 8. `anchr asm unitig` 命令（2026-08-11，借鉴 BCALM graph3）

新增独立命令 `anchr asm unitig`（**不从 `asm contig` 加开关**）：不做种子
...
（contig = tadpole 兼容，unitig = 严格图压缩）：
...
注意：[asm-contig.md](asm-contig.md) §7 曾称 `TadpoleTable::build` 是
"`pgr::libs::kmer::build_table`
同款模式"——实际并不同款：pgr 是 per-seq 打包 key + 并行基数排序 +
分组计数（FastK/Myers 骨架），anchr 是 HashMap + collect/reduce。
asm-contig.md §7 的 radix 评估（20k Lambda 下 radix 更慢）是小规模结论，本节的
...
minimizer 分桶（§13）的优先级降低——内存有界化（阶段 A）
成为更相关的下一步。
...
* **per-chunk 去重 + 树状合并**：每个 chunk 发射后立即 `count_keys`
  ...
  无关（确定性保持）。此即 §13 阶段 A 的无盘版本，k-way
  合并正是其中标为"需要新增"的那块。
...
* `asm contig` 保留 tadpole 的 `mincontiglen` 自动默认 `max(124, 2k)`
  （tadpole 兼容语义，[asm-contig.md](asm-contig.md) §2；由 contig 命令显式传入）；
...
<!-- 以下内容并入自 `§13`（2026-08-15 文档合并） -->

## 13. Unitig 计数分桶调研（minimizer/super-mer）

## Unitig 计数分桶：已有基础设施调研与计划（设计）

> 2026-08-13。触发：本文 §9.5 指出长 k 下 radix 排序成本随
