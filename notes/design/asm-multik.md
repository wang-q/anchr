...
* 单级门禁（用户选定口径）之外，三个数据集**全链门禁已全部跑通**（QR 组
  multik auto 31..160 → olc → extend → anchor → 跨组 `asm olc --unitigs`
  merge，QUAST `--min-contig 10`，错装以 quast 为准；结果均入
  `results/model_org.md` §end-multiplicity 门控全链门禁）：
  * **G37**（7 组 MR）：**0 mis 保持**，N50 121382（+13 bp）、GF 98.674
    （+0.008pp）较 08-16 基线无回归，Dup 1.001 冗余更低；
  * **MG1655**（标准 5 组 MRX40P000/P001/P002 + MRX80P000/P001）：
    **0 mis**，N50 112.6K、GF 98.197，与 08-18 现行复现（c9da0ce 合入
    后 5 组无 cv）同代吻合（08-17 基线 118.7K/99.072 的差归因 c9da0ce
    extend 护栏，非端门控）；
  * **DH5alpha**（13 组 MR，无 cv 主口径）：**0 mis**，N50 99,473 与
    08-18 门禁无 cv 记录逐位一致、GF 97.800（-0.14pp）——门控在干净的
    DH5alpha 上无新增回归（cv 口径在现行 anchor 上会假拆真连接，此处
    不采用）。
  结论：端门控在三个基准集全链均 **0 mis 且无 N50/GF 新回归**。
