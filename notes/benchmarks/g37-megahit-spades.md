...
### K 上限 256 适配与高 K 实验（2026-08-16 晚）

* pgr 上游 `Kmer::MAX_K` 128→256（commit `20069ca`；radix_sort 固定 64
  字节缓冲的越界由上游 `508a9f3` 修复）；anchr 同步：CLI help `1..=128`→
  `1..=256`（multik/map/olc/contig/unitig）、`auto_ks` 长读序列跑到 256
  （51..231）、`refine::get_count` 固定 `[u8; 32]`→`Kmer::MAX_K/4`
  （k=141 时 key_bytes=36 越界 panic）、上限测试改 257。
* **高 K 单主实验（G37 MRX40P000，~450 bp reads）**：K160 N50 72.9K（>
  K128 的 44.9K）、K192 37.6K、K224 16.4K、K256 6.0K——k≥192 急剧碎片化。
* **端到端（7 组，主 K 31..128+160）首轮**：N50 83.8K→**121.5K**（+45%）
  但 Dup 1.002→**1.068**（39K 冗余）、GF 98.983→98.849——近似重复没被
  去重。
* **修复：`merge_overlapping_contigs` 的 identity 改为 banded 编辑距离**：
  原 zip 直接比较在 indel 后错位，把 99.72% 同源的近重复（如 G37 K160 链
  contig_5/6，一个含 108 bp 内部插入）虚判为 87%；banded（band=512）
  后正确合并。**K160 全链重跑：N50 121.5K、Dup 1.068→**1.000**、总长
  -39K、0 mis；QUAST GF 98.849 的 4.5K"缺口"（222290-226811）是短
  contig 漏报假象（bwa 口径仅 59 bp 未覆盖）。**
* **K160 已进模板**（`6_unitigs.tera.sh` multik/unitig 分支
  `KS` 含 160；bcalm 分支 KS_BCALM 保持 31..121）。MG1655 验证：
  GF 98.364→**98.523%**（+0.16 pp）、N50 持平、0 mis、Dup 1.002。
