...
* **端到端（7 组，主 K 31..128+160，其余参数同 §10 最后一行）**：N50
  83.8K→**121.5K**（+45%）、GF 98.983→98.849（-0.13 pp）、Dup 1.002→
  **1.068**、0 mis。N50 增益来自 K160 长 unitig，但冗余明显升高、GF 反降
  ——**不进模板**；若未来要用，需先解决跨主 K 近似重复去重（Dup）。
