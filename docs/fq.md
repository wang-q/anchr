...
`anchr fq` provides tools for manipulating **FASTQ** files.
...
```bash
anchr fq interleave [OPTIONS] <infiles>...
```
...
1.  **Interleave two FASTQ files into one**:
    ```bash
    anchr fq interleave R1.fq R2.fq -o interleaved.fq --fq
    ```

2.  **Generate dummy pairs from a single FASTA file**:
    ```bash
    anchr fq interleave R1.fa --name-prefix sample --start-index 1
    ```

3.  **Convert separate FASTA files to interleaved FASTQ**:
    ```bash
    anchr fq interleave R1.fa R2.fa --fq -o out.fq
    ```
...
```bash
anchr fq clump [OPTIONS] <infiles>...
```
...
1.  **Sort two paired FASTQ files**:
    ```bash
    anchr fq clump R1.fq.gz R2.fq.gz -o clumped.fq
    ```
...
6.  **Limit parallelism**:
    ```bash
    anchr fq clump R1.fq.gz R2.fq.gz -o out.fq --parallel 4
    ```
...
```bash
anchr fq to-fa [OPTIONS] <infiles>...
```
...
file. It is the inverse of `anchr fq interleave` and matches BBTools
...
3.  **Repair disordered pairs and orphaned reads**:
    ```bash
    anchr fq split disordered.fq.gz --repair -o r1.fq --outfile-2 r2.fq \
        --outfile-single s.fq
    ```
...
```bash
anchr fq sample [OPTIONS] <infile>
```
...
(k-mer contaminant filtering) use `anchr fq filter`; for sickle-style pure
quality trimming use `anchr fq trim-qual`.

```bash
anchr fq clean [OPTIONS] <infiles>...
```
...
1.  **Clean with the anchr pipeline defaults**:
    ```bash
    anchr fq clean R1.fq.gz R2.fq.gz --ref illumina_adapters.fa -o out.fq
    ```

2.  **Clean without a reference (bbduk `qtrim=r minlen=...` style)**:
    ```bash
    anchr fq clean unmerged.raw.fq.gz -o unmerged.trim.fq \
...
3.  **Mask matching k-mers instead of trimming**:
    ```bash
    anchr fq clean in.fq --ref illumina_adapters.fa --mask-kmers N -o out.fq
    ```
...
```bash
anchr fq filter [OPTIONS] <infiles>...
```
...
1.  **Filter adapter/artifact matches (anchr filter step)**:
    ```bash
    anchr fq filter trim.fq.gz --ref illumina_adapters.fa -k 27 -o filter.fq
    ```
...
check, in contrast to `anchr fq filter`, which matches reads against an
...
```bash
anchr fq s-filter [OPTIONS] <infile>
```
...
```bash
anchr fq range reads.fq read1 read2 -o out.fq
anchr fq range reads.fq "read1:10-100" -o out.fq
anchr fq range reads.fq -r names.txt -c 10 -o out.fq
...
removed (use `anchr fq clean`/`anchr fq filter` for adapter/contaminant trimming).
...
1.  **Single-end trimming**:
    ```bash
    anchr fq trim-qual in.fq -o out.fq
    ```

2.  **Paired-end with separate outputs and singles**:
    ```bash
    anchr fq trim-qual R1.fq R2.fq -o r1.fq --outfile-2 r2.fq --outfile-single s.fq
    ```

3.  **Paired-end interleaved output**:
    ```bash
    anchr fq trim-qual R1.fq R2.fq -o interleaved.fq
    ```
...
This is the k-mer-graph counterpart of [`anchr fq ec-overlap`](#ec-overlap),
which corrects paired reads from their overlap instead.

```bash
anchr fq ec-kmer [OPTIONS] <infiles>...
```
...
counterpart of [`anchr fq ec-kmer`](#ec-kmer): this command needs paired reads
with a true overlap, while `ec-kmer` corrects from the k-mer graph.

```bash
anchr fq ec-overlap [OPTIONS] <infiles>...
```
...
```bash
anchr fq extend [OPTIONS] <infiles>...
```
