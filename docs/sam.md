...
`anchr sam` provides tools for manipulating **SAM** alignment files.
...
```bash
anchr sam ihist [OPTIONS] <infile>
```
...
1.  **Insert-size histogram from a paired mapping (anchr 2_insert_size
    ...
    anchr sam ihist mapped.sam -o insert_size.ihist.txt
    ```
...
```bash
anchr sam to-rg [OPTIONS] <infile>
```
...
1.  **Convert mapped reads to ranges**:
    ```bash
    anchr sam to-rg mapped.sam > mapped.rg
    ```
...
2.  **Derive per-base coverage from an `asm map` SAM (anchr anchors step)**:
    ...
    anchr sam to-rg mapped.sam | pgr rg coverage stdin -m 2 -o cov.json
    ```
