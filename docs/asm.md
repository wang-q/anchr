...
`anchr asm` provides assembly-related tools: building contigs/unitigs from
...
seeded extension), see [`anchr asm unitig`](#unitig).
...
[`anchr asm contig`](#contig), whose seeded contig mode keeps extending
...
```bash
anchr asm unitig [OPTIONS] <infiles>...
```
...
3.  **Raise the minimum unitig length**:
    ```bash
    anchr asm unitig in.fq -o out.fasta --min-contig-len 500
    ```
...
1.  **Map reads back to an assembly (anchr anchors step)**:
    ```bash
    anchr asm map UT.fasta R1.fq.gz R2.fq.gz \
...
```bash
anchr asm ovlp [OPTIONS] <infiles>...
```
...
```bash
anchr asm layout <paf> <infiles>... -o layout.tsv
```
...
```bash
anchr asm cns <layout.tsv> <infiles>... -o contigs.fa
```
