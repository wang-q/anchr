...
#[test]
fn command_fq_range_bgzf_autobuilds_gzi() {
    // A BGZF input without a sibling `.gzi` works: `fq range` builds the
    // index automatically (no separate `pgr fa gz --index` / bgzip step).
    let input = "@r1\nACGTACGT\n+\n!!!!!!!!\n@r2\nTGCA\n+\n!!!!\n";
    let dir = tempfile::tempdir().unwrap();
    let in_path = dir.path().join("in.fq.gz");
    {
    ...
    // The `.gzi` index was auto-built next to the input.
    assert!(dir.path().join("in.fq.gz.gzi").is_file());
}
